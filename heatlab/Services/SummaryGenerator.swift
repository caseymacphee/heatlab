...
final class SummaryGenerator {
    ...
    private func buildPrompt(sessionWithStats: SessionWithStats, comparison: BaselineComparison) -> String {
        ...
        Class: \(heatSession.classType?.rawValue ?? "Heated Class")
...
struct SummaryGeneratorFallback {
    static func generateBasicSummary(for sessionWithStats: SessionWithStats, comparison: BaselineComparison) -> String {
        let stats = sessionWithStats.stats
        let heatSession = sessionWithStats.session
        
        let durationMinutes = Int(stats.duration / 60)
        let classType = heatSession.classType?.rawValue ?? "heated yoga"
        
        var summary = "You completed a \(durationMinutes)-minute \(classType) session at \(heatSession.roomTemperature)°F. "
