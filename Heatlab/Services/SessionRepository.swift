...
final class SessionRepository {
    ...
    func fetchSessionsWithStats(isPro: Bool) async throws -> SessionFetchResult {
        let allSessions = try await fetchAllSessionsWithStats()
        
        if isPro {
            return SessionFetchResult(visibleSessions: allSessions, allSessions: allSessions)
        }
        
        // Free tier: filter to last 7 days (inclusive of full 7th day)
        // Use start of day to ensure the entire 7th day is included
        ...
        let visibleSessions = allSessions.filter { $0.session.startDate >= cutoffDate }
        
        return SessionFetchResult(visibleSessions: visibleSessions, allSessions: allSessions)
    }
